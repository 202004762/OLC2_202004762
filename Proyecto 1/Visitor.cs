    ...
    class Visitor : AnalizadorLexicoBaseVisitor<Object>{
        ...
        public override Object VisitInicio([NotNull] AnalizadorLexicoParser.InicioContext context){
            if (context.listaInstr() != null) {
                return VisitListaInstr(context.listaInstr());
            }
            return true;
        ...
        public override Object VisitInstruccion([NotNull] AnalizadorLexicoParser.InstruccionContext context){
            ...
            }else if (context.declaracionMatriz() != null){
                resultado = Visit(context.declaracionMatriz());
        ...
        public override Object VisitDeclaracionVar([NotNull] AnalizadorLexicoParser.DeclaracionVarContext context){
            try{
                ...
                //Console.WriteLine($"Variable declarada: {nombreVariable}, Tipo: {tipoVariable}, Valor: {valor}");
                return true;
        ...
        public override Object VisitDeclaracionMatrizLleno([NotNull] AnalizadorLexicoParser.DeclaracionMatrizLlenoContext context){
            try{
                Entorno entorno = pilaEntornos.Peek();
                string nombreMatriz = context.ID().GetText();
                string tipoElemento = context.tipo().GetText();
                string tipoFinal = $"matriz<{tipoElemento}>";
                List<List<object>> matriz = new List<List<object>>();
                foreach (var filaContext in context.listaFilas().fila()){
                    List<object> fila = new List<object>();
                    foreach (var expr in filaContext.expr()){
                        object valor = Visit(expr);
                        string tipoValor = DeterminarTipo(valor);
                        if (tipoValor != tipoElemento){
                            erroresSemanticos.Add(new Errores("Semantico", $"Error: Los valores de la matriz deben ser del tipo '{tipoElemento}', pero se encontró '{tipoValor}'.", context.Start.Line, context.Start.Column));
                            return false;
                ...
                if (entorno.buscarVariable(nombreMatriz) != null){
                    erroresSemanticos.Add(new Errores("Semantico", $"Error: La matriz '{nombreMatriz}' ya fue declarada en este ámbito.", context.Start.Line, context.Start.Column));
                    return false;

                }

                /*Console.WriteLine("DEBUG MATRIZ:");
                ...
                entorno.guardarVariable(nombreMatriz, new Simbolos(nombreMatriz, tipoFinal, matriz.Cast<object>().ToList()));
                Console.WriteLine($"Matriz '{nombreMatriz}' declarada con tipo {tipoFinal} y {matriz.Count} filas.");
                return true;

            }catch (Exception e){
                erroresSemanticos.Add(new Errores("Semantico", $"Error en la declaración de matriz: {e.Message}", context.Start.Line, context.Start.Column));
                return false;
        ...
        public override Object VisitAppend([NotNull] AnalizadorLexicoParser.AppendContext context){
            try{
                ...
                if (tipoDestino.StartsWith("slice<")){
                ...
                }else if (tipoDestino.StartsWith("matriz<")){
                    string tipoFila = tipoDestino.Substring(7, tipoDestino.Length - 8);
                    if (destino.valor is not List<object> matriz){
                        erroresSemanticos.Add(new Errores("Semantico", $"Error interno: El valor de '{nombreDestino}' no es una matriz valida.", context.Start.Line, context.Start.Column));
                        return false;

                    }

                    if (context.expr().Length == 1){
                        object posibleFila = Visit(context.expr(0));
                        if (posibleFila is not List<object> fila){
                            erroresSemanticos.Add(new Errores("Semantico", $"Error: El argumento para append debe ser un slice valido.", context.Start.Line, context.Start.Column));
                            return false;

                        }

                        foreach (var item in fila){
                            if (DeterminarTipo(item) != tipoFila){
                                erroresSemanticos.Add(new Errores("Semantico", $"Error: No se puede agregar un slice con elementos de tipo incorrecto a la matriz '{nombreDestino}'.", context.Start.Line, context.Start.Column));
                                return false;
                    ...
                    erroresSemanticos.Add(new Errores("Semantico", $"Error: Para matrices, append solo acepta un slice como fila completa.", context.Start.Line, context.Start.Column));
                    return false;

                }else{
                    erroresSemanticos.Add(new Errores("Semantico", $"Error: append solo se puede usar con slices o matrices. '{nombreDestino}' es de tipo '{tipoDestino}'.", context.Start.Line, context.Start.Column));
                    return false;
        ...
        public override Object VisitMatrizUpdate([NotNull] AnalizadorLexicoParser.MatrizUpdateContext context){
            try{
                string nombre = context.ID().GetText();
                object filaIndexObj = Visit(context.expr(0));
                object colIndexObj = Visit(context.expr(1));
                object nuevoValor = Visit(context.expr(2));
                if (filaIndexObj is not int filaIndex || colIndexObj is not int colIndex){
                    erroresSemanticos.Add(new Errores("Semantico", "Los índices deben ser valores enteros.", context.Start.Line, context.Start.Column));
                    return null;
                ...
                Simbolos? simbolo = entorno.buscarVariable(nombre);
                if (simbolo == null){
                    erroresSemanticos.Add(new Errores("Semantico", $"La matriz '{nombre}' no ha sido declarada.", context.Start.Line, context.Start.Column));
                    return null;

                }

                if (!simbolo.tipo.StartsWith("matriz<") || simbolo.valor is not List<object> listaGeneral){
                    erroresSemanticos.Add(new Errores("Semantico", $"La variable '{nombre}' no es una matriz válida.", context.Start.Line, context.Start.Column));
                    return null;

                }

                if (listaGeneral.Count == 0 || listaGeneral[0] is not List<object>){
                    erroresSemanticos.Add(new Errores("Semantico", $"El valor de '{nombre}' no es una lista de listas (matriz).", context.Start.Line, context.Start.Column));
                    return null;

                }

                List<List<object>> matriz = listaGeneral.Cast<List<object>>().ToList();

                if (filaIndex < 0 || filaIndex >= matriz.Count){
                    erroresSemanticos.Add(new Errores("Semantico", $"Índice de fila fuera de rango: {filaIndex}.", context.Start.Line, context.Start.Column));
                    return null;

                }

                List<object> fila = matriz[filaIndex];
                if (colIndex < 0 || colIndex >= fila.Count){
                    erroresSemanticos.Add(new Errores("Semantico", $"Índice de columna fuera de rango: {colIndex}.", context.Start.Line, context.Start.Column));
                    return null;

                }

                string tipoEsperado = simbolo.tipo.Substring(7, simbolo.tipo.Length - 8);
                string tipoValor = DeterminarTipo(nuevoValor);
                if (tipoEsperado != tipoValor){
                    erroresSemanticos.Add(new Errores("Semantico", $"El valor asignado es de tipo '{tipoValor}', se esperaba '{tipoEsperado}'.", context.Start.Line, context.Start.Column));
                    return null;

                }

                fila[colIndex] = nuevoValor;
                return true;

            }catch (Exception e){
                erroresSemanticos.Add(new Errores("Semantico", $"Error al asignar un valor a la matriz: {e.Message}", context.Start.Line, context.Start.Column));
                return null;
        ...
        private string DeterminarTipo(Object valor){
            try{
                if (valor is List<object> lista && lista.Count > 0 && lista[0] is List<object>) {
                    string tipoInterno = DeterminarTipo(((List<object>)lista[0])[0]);
                    return $"matriz<{tipoInterno}>";
                }
        ...
        public override object VisitInstruccion_funcion([NotNull] AnalizadorLexicoParser.Instruccion_funcionContext context){
            ...
            {
                erroresSemanticos.Add(new Errores("Semantico", $"La función '{nombreFuncion}' ya fue declarada.", context.Start.Line, context.Start.Column));
                return null;
            }
        ...
        public override object VisitLlamada_funcion([NotNull] AnalizadorLexicoParser.Llamada_funcionContext context){
            string nombreFuncion = context.ID().GetText();
            Entorno entorno = pilaEntornos.Peek();
            Simbolos? simbolo = entorno.buscarVariable(nombreFuncion);

            if (simbolo != null && simbolo.valor is Funcion funcion)
            {
                // Crear un nuevo entorno para la función
                Entorno entornoFuncion = new Entorno("Funcion_" + nombreFuncion, entorno);

                // Verificamos si hay parámetros para pasar
                if (context.lista_parametros() != null)
                {
                    var expresiones = context.lista_parametros().expr();
                    var tipos = context.lista_parametros().tipo();

                    if (expresiones.Length != funcion.parametros.Count - 1) // -1 por el return
                    {
                    ...
                    {
                        if (kvp.Key.StartsWith("retornar_"))
                            continue;

                        object valor = Visit(expresiones[index]);
                        string tipoValor = DeterminarTipo(valor);
                        string tipoEsperado = tipos[index].GetText();
                        ...
                        entornoFuncion.guardarVariable(kvp.Key, new Simbolos(kvp.Key, tipoValor, valor));
                        index++;
                    }
                }

                // Agregar variable de retorno
                entornoFuncion.guardarVariable(funcion.retorno.identificador, funcion.retorno);

                // Ejecutar instrucciones dentro del entorno de la función
                pilaEntornos.Push(entornoFuncion);
                VisitListaInstr(funcion.instrucciones);
                pilaEntornos.Pop();
                pilaEntornos.Peek().punteroASiguiente = null;

                if (funcion.retorno != null && funcion.retorno.valor != null)
                {
            ...
            {
                erroresSemanticos.Add(new Errores("Semantico", $"La función '{nombreFuncion}' no existe en el entorno actual.", context.Start.Line, context.Start.Column));
                return null;
            }
        ...
        public override object VisitReturnInstr([NotNull] AnalizadorLexicoParser.ReturnInstrContext context){
            object valorRetorno = Visit(context.expr());
            string tipoValor = DeterminarTipo(valorRetorno);

            // Buscar el entorno actual hacia arriba para encontrar la variable de retorno
            for (Entorno? ent = pilaEntornos.Peek(); ent != null; ent = ent.punteroAPadre)
            {
                foreach (var variable in ent.variables)
                {
                    if (variable.Key.StartsWith("retorno_"))
                    {
                        ...
                        if (tipoEsperado != tipoValor)
                        {
                            erroresSemanticos.Add(new Errores("Semantico",
                                $"El tipo de retorno es '{tipoValor}', pero se esperaba '{tipoEsperado}'.",
                        ...
                        Console.WriteLine($"DEBUG RETURN: Se asignó valor de retorno -> {valorRetorno}");
                        return true;
                    }
                }
            }
            ...
            erroresSemanticos.Add(new Errores("Semantico",
                $"Instrucción 'return' usada fuera de una función.",
                context.Start.Line, context.Start.Column));
            return null;
        ...
        public override Object VisitMultiplicacionYdivision([NotNull] AnalizadorLexicoParser.MultiplicacionYdivisionContext context){
            try{
                ...
                if (operador == "*"){
                    ...
                    erroresSemanticos.Add(new Errores("Semantico", "El operador % solo se permite entre enteros.", context.Start.Line, context.Start.Column));
                    return null;
        ...
        public override Object VisitMatrizAccess([NotNull] AnalizadorLexicoParser.MatrizAccessContext context){
            try{
                string nombre = context.ID().GetText();
                object filaIndexObj = Visit(context.expr(0));
                object colIndexObj = Visit(context.expr(1));
                ...
                if (filaIndexObj is not int filaIndex || colIndexObj is not int colIndex){
                    erroresSemanticos.Add(new Errores("Semantico", "Los índices de acceso a la matriz deben ser enteros.", context.Start.Line, context.Start.Column));
                    return null;
                ...
                if (!simbolo.tipo.StartsWith("matriz<") || simbolo.valor is not List<object> matriz){
                    erroresSemanticos.Add(new Errores("Semantico", $"La variable '{nombre}' no es una matriz.", context.Start.Line, context.Start.Column));
                    return null;
                ...
                if (matriz[filaIndex] is not List<object> fila || colIndex < 0 || colIndex >= fila.Count){
                    erroresSemanticos.Add(new Errores("Semantico", $"Índice de columna fuera de rango: {colIndex}.", context.Start.Line, context.Start.Column));
                    return null;

                }

                return ((List<object>)fila)[colIndex];

            }catch (Exception e){
                erroresSemanticos.Add(new Errores("Semantico", $"Error al acceder a la matriz: {e.Message}", context.Start.Line, context.Start.Column));
                return null;
